...
class Followup_reason extends CI_Controller{
	...
	public function __construct()
	{
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."/followup_reason";
    ...
    { 
        if($this->input->post('submit'))
        {
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                if($inserted_id > 0)
                {    
                    $this->session->set_flashdata('success_message',"Followup Reason Added Successfully.");
