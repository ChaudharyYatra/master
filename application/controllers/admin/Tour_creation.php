...
class Tour_creation extends CI_Controller{
	...
	{
        $fields = "tour_creation.*,package_type.package_type,package_type.id as pid";
        $this->db->where('tour_creation.is_deleted','no');
        // $this->db->where('tour_creation.is_active','yes');
		$this->db->order_by('CAST(tour_number AS DECIMAL(10,6)) ASC');
        $this->db->join("package_type", 'tour_creation.package_type=package_type.id','left');
        ...
        // $package_type = $this->master_model->getRecords('tour_creation');
    ...
    {   
        ...
        if($this->input->post('submit'))
        {
            ...
            {
                $this->db->where('is_active','yes');
                $tourNo_check = $this->master_model->getRecords('tour_creation',array('is_deleted'=>'no','tour_number'=>trim($this->input->post('tour_number')),'package_type'=>trim($this->input->post('package_type'))));
        ...
        // ====================PDF add ============================================
            ...
            //    echo '3';
               ...
               $file_name_to_dispaly_pdf =  $this->config->item('project_name').round(microtime(true)).str_replace(' ','_',$file_name['name']);
                ...
                $inserted_id = $this->master_model->insertRecord('tour_creation',$arr_insert,true);
        ...
        $zone_info = $this->master_model->getRecords('zone_master');
         ...
         $packages_tour_type = $this->master_model->getRecords('tour_creation');
  ...
  public function active_inactive($id,$type)
    {
        ...
        {   
            ...
            if($this->master_model->updateRecord('tour_creation',$arr_update,array('id' => $id)))
            {
    ...
    public function delete($id)
    {
        ...
        {   
            ...
            if($this->master_model->updateRecord('tour_creation',$arr_update,$arr_where))
            {
...
//============================================================================================================
            ...
            else
            {
                ...
                $arr_update = array(
                    ...
                    $arr_where     = array("id" => $id);
                    $this->master_model->updateRecord('tour_creation',$arr_update,$arr_where);
                    if($id > 0)
                    {
