...
class Delivery_information extends CI_Controller{

	public function __construct()
	{
		parent::__construct();
	    $this->arr_view_data = [];
        // if($this->session->userdata('nabcons_id')=="") 
        // { 
        //         redirect(base_url().'webmanager/login'); 
        // }
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."/delivery_information";
        $this->module_title       = "Delivery Information";
        $this->module_url_slug    = "delivery_information";
        $this->module_view_folder = "delivery_information/";    
	...
	{  
        ...
        $arr_data = $this->master_model->getRecords('delivery_information');
    ...
    {   
        ...
        if($this->input->post('submit'))
        {
            ...
            $this->form_validation->set_rules('delivery_information', 'Delivery Information', 'required');
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $inserted_id = $this->master_model->insertRecord('delivery_information',$arr_insert,true);
                               
                if($inserted_id > 0)
                {    
                    $this->session->set_flashdata('success_message',"Delivery Information Added Successfully.");
  ...
  public function active_inactive($id,$type)
    {
        ...
        {   
            ...
            if($this->master_model->updateRecord('delivery_information',$arr_update,array('id' => $id)))
            {
    ...
    public function delete($id)
    {
        ...
        {   
            ...
            if($this->master_model->updateRecord('delivery_information',$arr_update,$arr_where))
            {
    ...
    public function edit($id)
    {
        ...
        {   
            ...
            if($this->input->post('submit'))
            {
                ...
                if($this->form_validation->run() == TRUE)
                {
                   $delivery_information = trim($this->input->post('delivery_information'));
                   
                    $arr_update = array(
                        'delivery_information' => $delivery_information
                    );
                    $arr_where     = array("id" => $id);
                   $this->master_model->updateRecord('delivery_information',$arr_update,$arr_where);
                    if($id > 0)
                    {
