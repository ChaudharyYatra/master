...
class Terms_conditions extends CI_Controller{
	...
	public function __construct()
	{
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."/terms_conditions";
        $this->module_title       = "Terms Conditions";
        $this->module_url_slug    = "terms_conditions";
        $this->module_view_folder = "terms_conditions/";    
	...
	{  
        ...
        $arr_data = $this->master_model->getRecords('terms_conditions');
    ...
    {   
        ...
        if($this->input->post('submit'))
        {
            ...
            if(!empty($arr_data))
            {
                $this->session->set_flashdata('error_message'," Not Added. Terms & Conditions is already added. You can add only one Terms & Conditions.");
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $config['upload_path']   = './uploads/terms_conditions/';
                ...
                $inserted_id = $this->master_model->insertRecord('terms_conditions',$arr_insert,true);
  ...
  public function active_inactive($id,$type)
    {
	  $id=base64_decode($id);
        ...
        {   
            ...
            if($this->master_model->updateRecord('terms_conditions',$arr_update,array('id' => $id)))
            {
    ...
    public function delete($id)
    {
        ...
        {   
            ...
            if($this->master_model->deleteRecord('terms_conditions',$arr_where))
            {
        ...
        {   
            ...
            if($this->input->post('submit'))
            {
                ...
                if($this->form_validation->run() == TRUE)
                {
                   ...
                   $old_img_name = $this->input->post('old_img_name');
                    ...
                    {   
                        $file_name = $this->upload->data();
                        $filename = $file_name_to_dispaly;
                        if($old_img_name!='') unlink('./uploads/terms_conditions/'.$old_img_name);
                    }
                    else
                    {
                        $filename = $this->input->post('image_name',TRUE);
                ...
                else
                {
                    ...
                    $arr_where     = array("id" => $id);
                   $this->master_model->updateRecord('terms_conditions',$arr_update,$arr_where);
                    if($id > 0)
                    {
