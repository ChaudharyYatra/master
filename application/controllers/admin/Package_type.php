...
class Package_type extends CI_Controller{
	...
	public function __construct()
	{
        ...
        $this->module_url_path    =  base_url().$this->config->item('admin_panel_slug')."/package_type";
        $this->module_title       = "Package Type";
        $this->module_url_slug    = "package_type";
        $this->module_view_folder = "package_type/";    
	...
	{  
        ...
        $arr_data = $this->master_model->getRecords('package_type');
    ...
    {   
        ...
        if($this->input->post('submit'))
        {
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $this->db->where('is_deleted','no');
                $package_type_data = $this->master_model->getRecords('package_type');
                if(count($package_type_data) > 0)
                {
                    $this->session->set_flashdata('error_message',"Package Type ".$package_type." Already Exist.");
                    redirect($this->module_url_path.'/add');
                }
                
                $inserted_id = $this->master_model->insertRecord('package_type',$arr_insert,true);
  ...
  public function active_inactive($id,$type)
    {
	  $id=base64_decode($id);
        ...
        {   
            ...
            if($this->master_model->updateRecord('package_type',$arr_update,array('id' => $id)))
            {
    ...
    public function delete($id)
    {
        ...
        {   
            ...
            if($this->master_model->updateRecord('package_type',$arr_update,$arr_where))
            {
            ...
            if($this->input->post('submit'))
            {
                ...
                if($this->form_validation->run() == TRUE)
                {
                   $package_type = trim($this->input->post('package_type'));
                    ...
                    $arr_where     = array("id" => $id);
                   $this->master_model->updateRecord('package_type',$arr_update,$arr_where);
                    if($id > 0)
                    {
