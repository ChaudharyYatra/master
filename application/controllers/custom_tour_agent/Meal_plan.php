...
class Meal_plan extends CI_Controller{
	...
	public function __construct()
	{
        ...
        $this->module_url_path    =  base_url().$this->config->item('custom_tour_agent_panel_slug')."custom_tour_agent/meal_plan";
        $this->module_title       = "Meal Plan";
        $this->module_url_slug    = "meal_plan";
        $this->module_view_folder = "meal_plan/";    
	...
	{  
        ...
        $arr_data = $this->master_model->getRecords('meal_plan');
    ...
    {   
        ...
        if($this->input->post('submit'))
        {
            ...
            $this->form_validation->set_rules('meal_plan_name', 'meal_plan_name', 'required');
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $this->db->where('is_deleted','no');
                $meal_plan_name_exist_data = $this->master_model->getRecords('meal_plan');
                if(count($meal_plan_name_exist_data) > 0)
                {
                    $this->session->set_flashdata('error_message',"Meal Plan Name ".$meal_plan_name." Already Exist.");
  ...
  public function active_inactive($id,$type)
    ...
    public function delete($id)
    {
        ...
        {   
            ...
            if($this->master_model->updateRecord('meal_plan',$arr_update,$arr_where))
            {
    ...
    public function edit($id)
    {
        ...
        {   
            ...
            if($this->input->post('submit'))
            {
                ...
                if($this->form_validation->run() == TRUE)
                {
                   $meal_plan_name = trim($this->input->post('meal_plan_name'));
                    ...
                    $arr_where     = array("id" => $id);
                   $this->master_model->updateRecord('meal_plan',$arr_update,$arr_where);
                    if($id > 0)
                    {
