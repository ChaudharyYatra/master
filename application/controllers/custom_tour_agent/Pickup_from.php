...
class Pickup_from extends CI_Controller{
	...
	public function __construct()
	{
        ...
        $this->module_url_path    =  base_url().$this->config->item('custom_tour_agent_panel_slug')."custom_tour_agent/pickup_from";
        $this->module_title       = "Pick Up From";
        $this->module_url_slug    = "pickup_from";
        $this->module_view_folder = "pickup_from/";    
	...
	{  
        ...
        $arr_data = $this->master_model->getRecords('pick_up_from');
    ...
    {   
        ...
        if($this->input->post('submit'))
        {
            ...
            $this->form_validation->set_rules('pick_up_name', 'pick_up_name', 'required');
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $this->db->where('is_deleted','no');
                $pick_up_name_exist_data = $this->master_model->getRecords('pick_up_from');
                if(count($pick_up_name_exist_data) > 0)
                {
                    $this->session->set_flashdata('error_message',"Pick Up name ".$pick_up_name." Already Exist.");
  ...
  public function active_inactive($id,$type)
    ...
    public function delete($id)
    {
        ...
        {   
            ...
            if($this->master_model->updateRecord('pick_up_from',$arr_update,$arr_where))
            {
    ...
    public function edit($id)
    {
        ...
        {   
            ...
            if($this->input->post('submit'))
            {
                ...
                if($this->form_validation->run() == TRUE)
                {
                   $pick_up_name = trim($this->input->post('pick_up_name'));
                    ...
                    if(count($pick_up_name_exist_data) > 0)
                    {
                        $this->session->set_flashdata('error_message',"Pick Up From ".$pick_up_name." Already Exist.");
                        redirect($this->module_url_path.'/add');
                    }
                    ...
                    $arr_where     = array("id" => $id);
                   $this->master_model->updateRecord('pick_up_from',$arr_update,$arr_where);
                    if($id > 0)
                    {
