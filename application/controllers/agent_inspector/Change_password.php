...
class Change_password extends CI_Controller {
	...
	function __construct() {
        ...
        $this->module_url_path    =  base_url().$this->config->item('agent_inspector_panel_slug')."agent_inspector/change_password";
     ...
     public function change_password()
     {
        ...
        if($this->input->post('submit'))
                ...
                $id=$this->session->userdata('agent_inspector_sess_id');
                 ...
                 {                  
                    ...
                    if($existed_password == $old_pass)
                    {
                         ...
                         $this->master_model->updateRecord('agent_inspector',$arr_update,$arr_where);
                         if($id > 0)
                         {
                             $this->session->set_flashdata('success_message',$this->module_title." Successfully.");
                              redirect('agent_inspector/login/logout');
                         }
                         else
                         {
         ...
         $this->arr_view_data['module_url_path_dashboard'] = $this->module_url_path_dashboard;
