...
class Inter_all_traveller_info extends CI_Controller {
	...
	function __construct() {
        ...
        $this->module_url_path    =  base_url().$this->config->item('agent_panel_slug')."/inter_all_traveller_info";
        $this->module_inter_booking_enquiry    =  base_url().$this->config->item('agent_panel_slug')."/international_booking_enquiry";
        $this->inter_seat_type_room_type    =  base_url().$this->config->item('agent_panel_slug')."/inter_seat_type_room_type";
        $this->module_title       = "International All Traveller Info";
        $this->module_url_slug    = "inter_all_traveller_info";
        $this->module_view_folder = "inter_all_traveller_info/";
        $this->arr_view_data = [];
	 }
        ...
        $this->arr_view_data['module_inter_booking_enquiry']        = $module_inter_booking_enquiry;
     ...
     public function add($iid="")
        ...
        $inter_booking_basic_info_data = $this->master_model->getRecords('inter_booking_basic_info');
        ...
        //          $inserted_id = $this->master_model->insertRecord('inter_all_traveller_info',$arr_insert,true);
        //         }               
        //          if($inserted_id > 0)
        //          {
        ...
        //         $mrandmrs  = $this->input->post('mrandmrs');
        //         $first_name  = $this->input->post('first_name');
        //         // print_r($this->input->post('first_name')); die;
        //         $middle_name  = $this->input->post('middle_name');
        //         $last_name  = $this->input->post('last_name'); 
        //         $dob  = $this->input->post('dob'); 
        //         $relation  = $this->input->post('relation');
        ...
        //          {
        //              $this->session->set_flashdata('success_message',ucfirst($this->module_title)." Added Successfully.");
        //              redirect($this->inter_seat_type_room_type.'/add/'.$iid);
        //          }
        //          else
        //          {
        //              $this->session->set_flashdata('error_message',"Something Went Wrong While Adding The ".ucfirst($this->module_title).".");
     ...
     {  
      ...
      $id=$this->session->userdata('agent_sess_id');
        ...
        //  print_r($agent_booking_enquiry_data); die;
         ...
         $this->db->where('is_deleted','no');
         $this->db->where('domestic_enquiry_id',$iid);
         $arr_all_traveller = $this->master_model->getRecords('inter_all_traveller_info');

         $this->db->order_by('id','ASC');
         $this->db->where('is_deleted','no');
         $this->db->where('domestic_enquiry_id',$iid);
         $arr_all_traveller_addr = $this->master_model->getRecord('inter_all_traveller_info');
      ...
      if($this->input->post('booknow_submit'))
      {
         ...
         //  { 
             ...
             for($i=0; $i<$c; $i++){
                // echo $i;
             //    $_FILES['image_name']['name'] = $_FILES['image_name']['name'][$i];
             //    $file_name = $_FILES['image_name']['name'];
             //   // $_FILES['image_name']['name'] = $_FILES['image_name']['name'][$i];
             ...
             //  print_r($tid); die;
              if(!empty($arr_all_traveller)){
                 ...
                 $inserted_id = $this->master_model->updateRecord('inter_all_traveller_info',$arr_update,$arr_where);
                 }else{
                     $inserted_id = $this->master_model->insertRecord('inter_all_traveller_info',$arr_update,true);
                 }
              ...
              {
                  $this->session->set_flashdata('success_message',ucfirst($this->module_title)." Added Successfully.");
                  redirect($this->inter_seat_type_room_type.'/edit/'.$iid);
              }
              else
              {
                  $this->session->set_flashdata('error_message',"Something Went Wrong While Adding The ".ucfirst($this->module_title).".");
    ...
    //   $this->arr_view_data['module_booking_enquiry'] = $this->module_booking_enquiry;
      $this->arr_view_data['inter_seat_type_room_type'] = $this->inter_seat_type_room_type;
